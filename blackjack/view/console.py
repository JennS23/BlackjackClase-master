...
class UIConsola:
    ...
    def ejecutar_turno_de_la_casa(self):
        ...
        if self.blackjack.cupier.mano.es_blackjack():
          print("La casa tiene Blackjack. ¡Has perdido!")
          self.finalizar_juego(False)
          return

        while self.blackjack.cupier.mano.calcular_valor() <= 16:
          self.blackjack.repartir_carta_a_la_casa()
          self.mostrar_manos(self.blackjack.cupier.mano, self.blackjack.jugador.mano)

        valor_casa = self.blackjack.cupier.mano.calcular_valor()
        if valor_casa > 21:
          print("La casa se pasó de 21. ¡Has ganado!")
          self.finalizar_juego(True)
        else:
          print("La casa se planta.")
          self.finalizar_juego(self.blackjack.jugador_gano())
